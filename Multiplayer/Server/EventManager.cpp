...
EventManager::DeathEvent::DeathEvent(int id, int priority, Character* player, EventManager* manager)
    : Event(EVENT_TYPE_DEATH, id, priority, manager) {

    this->player = player;
}

void EventManager::DeathEvent::Death() {
    std::cout << "Player " << this->getId() << " died." << std::endl;
...
void EventManager::RegisterDeathEvent(int id, int priority, Character* player) {
    ...
    DeathEvent event = DeathEvent(id, priority, player, this);
